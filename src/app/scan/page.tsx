...
export default function ScanPage() {
  ...
  // Clean up temporary image object URLs
  ...
  const handleQrPayload = useCallback(
    (rawPayload: string) => {
      ...
      if (!roomCode) {
        ...
        setErrorMessage('Invalid TempLink QR. Please upload or scan a QR code generated by TempLink.');
      ...
      // Track product event (strictly no sensitive payload storage)
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
        setErrorMessage('Camera permission was denied. You can upload a QR image from your gallery.');
      } else {
  ...
  const processQrImageFile = (file: File) => {
    ...
    if (!file || !file.type.startsWith('image/')) {
      toast('Please choose a valid image file (PNG, JPG, WebP)', 'error');
      return;
    }
    ...
    if (file.size > MAX_FILE_SIZE) {
      setScannerState('error');
      setErrorMessage('Image is too large (maximum 10 MB). Please choose a smaller image.');
    ...
    // Create temporary local object URL for preview only
    clearPreviewUrl();
    ...
    img.onload = () => {
      try {
        ...
        // Pass 1: Scaled decode for high-res screenshots and mobile photos
        ...
        if (code && code.data) {
          ...
          setIsProcessingImage(false);
          setScannerState('error');
          setErrorMessage('No QR code was found in this image. Make sure the QR code is clearly visible and try again.');
      ...
      <BlurFade delay={0.1} className="w-full max-w-[500px] mt-2">
        ...
        <div className="relative glass-panel p-6 sm:p-8 rounded-3xl flex flex-col items-center gap-6 border border-white/10 bg-[#080B12]/90 shadow-2xl overflow-hidden">
          ...
          {scanMode === 'camera' ? (
            ...
            /* UPLOAD FROM GALLERY VIEWPORT (With Image Preview & In-Browser Decoding) */
            <div
              onDragOver={(e) => {
                e.preventDefault();
                setIsDragging(true);
            ...
            >
              ...
              {previewImageUrl && (
                <img
                  ...
                  className="absolute inset-0 w-full h-full object-contain opacity-35 filter blur-[1px]"
                />
              )}

              {isProcessingImage ? (
                <div className="relative z-10 flex flex-col items-center gap-3 bg-black/60 p-4 rounded-2xl backdrop-blur-sm">
                ...
                <div className="relative z-10 flex flex-col items-center gap-3 bg-black/70 p-4 rounded-2xl backdrop-blur-sm animate-fade-in">
                  <span className="material-symbols-outlined text-4xl text-emerald-400 animate-bounce">
                    check_circle
                  </span>
                  <span className="text-sm font-bold text-white">QR Code Detected!</span>
                ...
                <div className="relative z-10 flex flex-col items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-[11px] text-slate-400 block">
                      Supports screenshots, photos, and files (Max 10 MB)
                    </span>
                  </div>
                  <button
                    type="button"
          ...
          {errorMessage && (
            <div className="p-3 bg-red-950/40 border border-red-500/30 rounded-xl text-xs text-red-200 text-center w-full max-w-[320px] flex flex-col gap-2">
              ...
              {scanMode === 'upload' && (
                <div className="flex gap-2 justify-center mt-1">
                  <button
                    type="button"
                    onClick={() => fileInputRef.current?.click()}
                    className="btn-primary text-[10px] font-bold py-1 px-2.5 rounded-lg uppercase"
                  >
                    Try Another Image
                  </button>
                  <button
                    ...
                    className="btn-ghost text-[10px] font-semibold py-1 px-2.5 rounded-lg"
                  >
                    Scan with Camera
                  </button>
                </div>
