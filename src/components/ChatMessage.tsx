...
export default function ChatMessage({ message, isMe }: ChatMessageProps) {
  ...
  if (isMe) {
    return (
      <div className="flex flex-col gap-1 self-end max-w-[85%] sm:max-w-[75%] items-end animate-fade-in">
        <div className="bg-gradient-to-r from-[#6366F1] to-[#4F46E5] text-white rounded-2xl rounded-tr-sm px-4 py-2.5 shadow-[0_4px_15px_rgba(99,102,241,0.25)] font-normal text-xs sm:text-sm leading-relaxed border border-white/15">
          {message.file ? (
  ...
  return (
    <div className="flex flex-col gap-1 self-start max-w-[85%] sm:max-w-[75%] animate-fade-in">
      <span className="text-[10px] font-semibold text-primary-light ml-1 uppercase tracking-wider">
        {message.senderName}
      </span>
      <div className="bg-[#0D111A] border border-white/10 text-slate-200 rounded-2xl rounded-tl-sm px-4 py-2.5 shadow-sm text-xs sm:text-sm leading-relaxed">
        {message.file ? (
          <div className="flex flex-col gap-2">
            {message.file.type?.startsWith('image/') && (
              <img
